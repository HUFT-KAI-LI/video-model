...
if [[ -n "$(git status --porcelain)" ]]; then
  echo "Refusing D2 generation from a dirty worktree" >&2
  exit 2
fi

commit="$(git rev-parse --short=12 HEAD)"
root="validation/history_subsets/screen_${commit}"
mkdir -p "$root"

pids=()
for gpu in 0 1 2 3; do
  CUDA_VISIBLE_DEVICES="$gpu" OMP_NUM_THREADS=8 TOKENIZERS_PARALLELISM=false \
    .venv/bin/python scripts/run_history_subset_interaction.py \
...
if [[ "$failed" -ne 0 ]]; then
  echo "At least one D2 shard failed; inspect $root/shard*.log" >&2
  exit 1
fi

.venv/bin/python scripts/analyze_history_subset_interaction.py \
  --results "$root/shard0.json" "$root/shard1.json" "$root/shard2.json" "$root/shard3.json" \
  --output "$root/analysis.json"

echo "D2 analysis: $root/analysis.json"
