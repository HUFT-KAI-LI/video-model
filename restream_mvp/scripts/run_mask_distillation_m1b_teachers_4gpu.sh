...
if [[ -n "$(git status --porcelain)" ]]; then
  echo "Refusing M1-B teacher generation from a dirty worktree" >&2; exit 2
fi
commit="$(git rev-parse --short=12 HEAD)"
root="validation/mask_distillation_m1b/teachers_${commit}"
mkdir -p "$root"
pids=()
for gpu in 0 1 2 3; do
  CUDA_VISIBLE_DEVICES="$gpu" OMP_NUM_THREADS=8 TOKENIZERS_PARALLELISM=false \
    ...
    --manifest validation/mask_distillation_m1b_teacher_manifest.json \
...
if [[ "$failed" -ne 0 ]]; then
  echo "At least one M1-B teacher shard failed" >&2; exit 1
fi
echo "M1-B teachers: $root"
